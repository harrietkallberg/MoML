...
data_without_genre <- data_raw[data_raw$music_genre == "", ]
...
for (col in compare_columns) {
  ...
  data_with_both <- data_with_genre[!is.na(data_with_genre[[col]]) & is.finite(data_with_genre[[col]]), ]
  data_with_only_col <- data_without_genre[!is.na(data_without_genre[[col]]) & is.finite(data_without_genre[[col]]), ]
  data_with_only_genre <- data_with_genre[is.na(data_with_genre[[col]]) | !is.finite(data_with_genre[[col]]), ]
  data_without_both <- data_without_genre[is.na(data_without_genre[[col]]) | !is.finite(data_without_genre[[col]]), ]
  
  # Define the PDF file path for saving the plot
  pdf_path <- file.path(plots_dir, paste0("boxplot_", col, ".pdf"))
  ...
  p <- ggplot(data_with_both, aes(x = music_genre, y = get(col))) + 
    ...
    labs(
      ...
      y = col,
      caption = paste(
        "Missing genre but has", col, ":", nrow(data_with_only_col), "\n",
        "Missing", col, "but has genre:", nrow(data_with_only_genre), "\n",
        "Missing both:", nrow(data_without_both), "\n",
